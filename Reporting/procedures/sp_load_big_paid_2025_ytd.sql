...
try {
    ...
    if (rows_added > 0) {
        ...
        body = `SUCCESS: Loaded ${rows_added} new row(s) into reporting.details.CLAIMS_IMPORT from stage @reporting.details.REPORTS.`;
        returnMsg = 'SUCCESS: Loaded ' + rows_added + ' new row(s) into reporting.details.CLAIMS_IMPORT from stage @reporting.details.REPORTS.';
    } else {
        subject = 'Big Paid 2025 YTD Load: No New Rows';
        body = 'Load successfully ran: No new rows were loaded into reporting.details.CLAIMS_IMPORT. The data is up to date and no duplicates were found.';
